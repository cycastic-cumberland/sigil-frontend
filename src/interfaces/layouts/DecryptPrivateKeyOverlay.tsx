...
import {type ChangeEvent, type FC, type ReactNode, type SyntheticEvent, useEffect, useState} from "react";
...
const DecryptPrivateKeyOverlay: FC<{ children?: ReactNode | ReactNode[] }> = ({ children }) => {
    ...
    return !openDialog ? children : <div className={"w-full min-h-screen bg-background"}>
        { isDesktop ? <Dialog open={openDialog} onOpenChange={setOpenDialog} >
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    <DialogTitle>Enter your password</DialogTitle>
                    <DialogDescription>Enter your password to unlock session</DialogDescription>
                </DialogHeader>
                <DecryptionDialog isLoading={isLoading} onSave={onSubmit}/>
            </DialogContent>
        </Dialog> : <Drawer open={openDialog} onOpenChange={setOpenDialog}>
            <DrawerContent>
                <DrawerHeader>
                    <DrawerTitle>Enter your password</DrawerTitle>
                    <DrawerDescription>Enter your password to unlock session</DrawerDescription>
                </DrawerHeader>
                <div className={"p-5"}>
                    <DecryptionDialog isLoading={isLoading} onSave={onSubmit}/>
                </div>
